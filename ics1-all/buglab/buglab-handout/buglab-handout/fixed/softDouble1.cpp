...
{
    return (Exp(x) == (ui1 << 11) - 1) && (Fraction(x) & ((ui1 << 52) - 1)) != 0;
}
...
uint64_t add(uint64_t lhs, uint64_t rhs)
{
    ...
    if (ansexp >= ((ui1 << 11) - 1)) // overflow
        ans = INF;
    else
        ans = ansexp << 52 | (ansf & ((ui1 << 52) - 1));
...
{
    if (Exp(x) != 0)
        return ui1 << 52 | (x & ((ui1 << 52) - 1));
    else
        return (x & ((ui1 << 52) - 1)) << 1; // normalize subnormal
}
